...
Setup::Setup() :
	...
	motorDirPin(1),
	motorDirPinValue(0),
	motorMicrostepM0Pin(21),
	motorMicrostepM1Pin(22),
	motorMicrostepM2Pin(23),
	motorMicrostepFactor(0),
	limitSwitchPin(26),
	limitSwitchValue(1),
	laserOnValue(1),
...
void Setup::encodeProperties(std::vector<Property>& properties)
{
	...
	properties.push_back(Property("setup.motorMicrostepM0Pin", ToString(motorMicrostepM0Pin)));
	properties.push_back(Property("setup.motorMicrostepM1Pin", ToString(motorMicrostepM1Pin)));
	properties.push_back(Property("setup.motorMicrostepM2Pin", ToString(motorMicrostepM2Pin)));
	properties.push_back(Property("setup.motorMicrostepFactor", ToString(motorMicrostepFactor)));
	properties.push_back(Property("setup.limitSwitchPin", ToString(limitSwitchPin)));
	properties.push_back(Property("setup.limitSwitchValue", ToString(limitSwitchValue)));
...
void Setup::decodeProperties(const std::vector<Property>& properties)
{
	for (size_t iProp = 0; iProp < properties.size(); iProp++)
	{
		...
		else if (prop.name == "setup.motorMicrostepM0Pin")
		{
			motorMicrostepM0Pin = ToInt(prop.value);
		}
		else if (prop.name == "setup.motorMicrostepM1Pin")
		{
			motorMicrostepM1Pin = ToInt(prop.value);
		}
		else if (prop.name == "setup.motorMicrostepM2Pin")
		{
			motorMicrostepM2Pin = ToInt(prop.value);
		}
		else if (prop.name == "setup.motorMicrostepFactor")
		{
			motorMicrostepFactor = ToInt(prop.value);
		}
		else if (prop.name == "setup.limitSwitchPin")
		{
			limitSwitchPin = ToInt(prop.value);
		}
		else if (prop.name == "setup.limitSwitchValue")
		{
			limitSwitchValue = ToInt(prop.value);
		}
    ...
    // Validate the microstep factor is actually supported by pin assignments
    int maxMicrostepFactor = 0;
    if (motorMicrostepM0Pin != -1)
    {
        maxMicrostepFactor = 1;
        if (motorMicrostepM1Pin != -1)
        {
            maxMicrostepFactor = 3;
            if (motorMicrostepM2Pin != -1)
                maxMicrostepFactor = 5;
        }
    }
    if (motorMicrostepFactor > maxMicrostepFactor)
        motorMicrostepFactor = maxMicrostepFactor;
