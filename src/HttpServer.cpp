...
static void SavePreset(RequestInfo * reqInfo)
{
    ...
    std::string scanModeSelection = reqInfo->arguments[WebContent::SCANNING_MODE_SELECTION];
    if (!scanModeSelection.empty())
    {
        preset->scanMode = (ScanningMode) ToInt(scanModeSelection.c_str());
    }
...
static void SaveSetup(RequestInfo * reqInfo)
{
    ...
    std::string m0PinValue = reqInfo->arguments[WebContent::MICROSTEP_M0_PIN];
    if (!m0PinValue.empty())
    {
        setup->motorMicrostepM0Pin = ToInt(m0PinValue);
    }
    
    std::string m1PinValue = reqInfo->arguments[WebContent::MICROSTEP_M1_PIN];
    if (!m1PinValue.empty())
    {
        setup->motorMicrostepM1Pin = ToInt(m1PinValue);
    }
    
    std::string m2PinValue = reqInfo->arguments[WebContent::MICROSTEP_M2_PIN];
    if (!m2PinValue.empty())
    {
        setup->motorMicrostepM2Pin = ToInt(m2PinValue);
    }
    
    std::string microstepFactor = reqInfo->arguments[WebContent::MICROSTEPPING_FACTOR];
    if (!microstepFactor.empty())
    {
        setup->motorMicrostepFactor = ToInt(microstepFactor);
    }
    
    std::string motorLimitPin = reqInfo->arguments[WebContent::MOTOR_LIMIT_SWITCH_PIN];
    if (!motorLimitPin.empty())
    {
        setup->limitSwitchPin = ToInt(motorLimitPin);
    }
    
    std::string motorLimitValue = reqInfo->arguments[WebContent::MOTOR_LIMIT_SWITCH_VALUE];
    if (!motorLimitValue.empty())
    {
        setup->limitSwitchValue = ToInt(motorLimitValue);
    }
...
{
    Preset currPreset = PresetManager::get() -> getActivePreset ();
	MotorDriver::release();
	MotorDriver::getInstance(currPreset.scanMode);
